...
import static com.commax.control.R.styleable.CircularSeekBar_pointer_radius;

/*
 Example
  CircularSeekBar m_seekbar_level;
  m_seekbar_level = (CircularSeekBar) findViewById(R.id.circularSeekBar_control);
  m_seekbar_level.setCircleProgressGradientColor(0xff433fa3, 0xff4ec3e5); //add ftn
...
public class CircularSeekBar extends View {
	...
	protected static final int DEFAULT_MIN = 0; //yslee
	...
	protected void initAttributes(TypedArray attrArray) {
		...
		mPointerRadius = attrArray.getDimension(CircularSeekBar_pointer_radius, DEFAULT_POINTER_RADIUS * DPTOPX_SCALE);
	...
	*/
	...
	/**
	 * Sets the circle progress gradient color.
	 * 2016.08.18, Add by yslee :: user defined gradient color setting function
	 */
	public void setCircleProgressGradientColor(int startcolor, int endcolor) {

		//Dimming : {0xffed5c7e, 0xff504fae};
		//Warm    : {0xffe02f38, 0xfff8b54c};
		//Cool    : {0xff433fa3, 0xff4ec3e5};

		int[] colors = {startcolor, endcolor};  //color
		float[] colorPos = {0.0f, 1.0f};//color position

		mCircleProgressColor = startcolor;
		mCircleProgressPaint.setColor(mCircleProgressColor);

		LinearGradient shader = new LinearGradient(mCircleXRadius, mCircleYRadius, 0, mCircleYRadius*1.3f, colors, colorPos, Shader.TileMode.CLAMP);
	...
	public void setIsPointEnabled(boolean isPointEnabled) {
		if(isPointEnabled==false)	mPointerRadius = 0;
	}
	...
	public void setMax(int max) {
		if (!(max <= 0)) { // Check to make sure it's greater than zero
			if (max <= mProgress) {
				mProgress = DEFAULT_MIN; // If the new max is less than current progress, set progress to zero
				if (mOnCircularSeekBarChangeListener != null) {
