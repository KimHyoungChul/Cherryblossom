 ...
 */
public class More_FCU extends LinearLayout implements View.OnClickListener{

    String TAG = More_FCU.class.getSimpleName();
    ...
    int MSG_SEND_ID;
    //플러스 마이너스 버튼 누르고 재 입력 있을 수 있을 경우 해당 시간동안 제어 신호를 보내지 않고 기다린다.
    ...
    public More_FCU(Context context) {
        super(context);
        init(context);
    }

    public More_FCU(Context context, DeviceInfo devicedata) {
        super(context);
        Log.d(TAG , "More_FCU");
    ...
    public void init(Context context)
    {
        ...
        //UI Setting
        deviceUIsetting();
        //제어 변경된 후 변경된 PAM 응답이 있는지를 check 한다.
        startMonitor();
        //타이머 선언 (seek bar)
    ...
    public void deviceUIsetting()
    {
        ...
        try{
            ...
            if(deviceInfo.getValue2().equals(TypeDef.SWITCH_ON))
            {
                ...
                //0913 전원 OFF 일때 해당 seekbar disable 처리
                fcu_data_value.setTextColor(getResources().getColor(R.color.data_number_text));
                fcu_data_scale1.setTextColor(getResources().getColor(R.color.data_number_text));
                fcu_data_scale2.setTextColor(getResources().getColor(R.color.data_number_text));
                ic_dc_warm_cool_icon.setImageAlpha(255);
