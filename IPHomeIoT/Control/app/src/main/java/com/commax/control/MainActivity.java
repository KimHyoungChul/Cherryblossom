...
//        tablist_init();
    ...
    private void container_init(Context context) {
        ...
        if(!TypeDef.MENU_ADD_ENABLE) {
            btn_add.setVisibility(View.INVISIBLE); //add 버튼 숨기기
        }
            ...
            for (int i = 0; i < listsize; i++) {
                ...
                switch (device_data.nCardType) {
                    ...
                    case eDetectSensor:
                        ...
                        // 멀티 센서, 접점 센서 분리 처리
                        if(device_data.getCommaxDevice().equals(TypeDef.COMMAX_DEVICE_DETECTSENSOR))
                        {
                            Card_MultiSensor_PIR card_detectsensor = (Card_MultiSensor_PIR)CardManager.getInst().getDeviceCard(device_data);
                            // 멀티센서인데 sort 가 3개가 아니면 카드 UI를 그리지 않는다.
                            if(device_data.other_subcount == 2)
                            {
                        ...
                        else if(device_data.getCommaxDevice().equals(TypeDef.COMMAX_DEVICE_CONTACTSENSOR))
                        {
                            //접점 센서
                            //TODO 마그네틱 센서사 일반 적인 센서 룰 카드이므로 마그네틱 센서 카드 활용
    ...
    public void sendDeviceAddcancleCommand() {
        ...
        try{
            sendCommand("{\"command\":\"addCancel\"} ");
            Log.d(TAG, "send addCancel Command");
        }catch (Exception e){
            e.printStackTrace();
        }
    }
    ...
    public void addReport(final String add_raw) {
        ...
        Log.d(TAG, "addReport add_raw : " + add_raw);
        String rootUuid = dataManager.mysql_getRootUuid(add_raw);
        // sort 해서 방법, 비상에 대해서 UI 업데이트 하지 않기
        String subUuid = dataManager.mysql_getsubUuid(add_raw);
        String commax_device = dataManager.mysql_getCommaxDevice(add_raw);
        String nickname = "updated";

        Log.e(TAG, "commax_device : " + commax_device + " , rootUuid : " + rootUuid + " , subUuid : " + subUuid + " , nickname :  " + nickname);

        if(commax_device.equals(TypeDef.COMMAX_DEVICE_AWAYSWITCH) || commax_device.equals(TypeDef.COMMAX_DEVICE_PREVENTSWITCH))
        {
            //nothing
            Log.d(TAG, "방범 비상 등록됨 nothing show in UI");
        }
        else
        {
            if(!mbNeedTo_addReport)
            {
                ...
                waitDialogEnable(getResources().getString(R.string.message_dbupdate_string), true);
                //무선 디바이스 add되자마자 즐겨찾기 추가 되도록
                MainActivity.getInstance().dataManager.addFavoriteFile(subUuid,nickname);
...
*/
    ...
    */
    ...
    public void BtnGuide_Clicked(View v)
    {
        ...
        //TODO guide need get deviceinfosimple??
        getDeviceInfoSimple();
        try {
            Intent intent = new Intent();
            ...
            // 디바이스 추가시 토스트 메세지 뜨기때문에 flag 를 false 로 설정
            activity_on = false;
...
//            customdialog_one_button.show();
    ...
    public void Tab_able_disable()
    {
        ...
        try
        {
            /*if(mCategory_eLight.isEmpty())
            {
                //TODO array 가 선언되기 전인지 아닌지 판단하기
                Log.d(TAG, "not data init yet");
            }
            else
            {*/
                if(mCategory_eLight.size() == 0)
