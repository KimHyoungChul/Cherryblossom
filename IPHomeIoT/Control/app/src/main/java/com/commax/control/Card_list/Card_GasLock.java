 ...
 */
public class Card_GasLock extends LinearLayout implements View.OnClickListener {

    static final String TAG = "Card_GasLock";
    ...
    public Card_GasLock(Context context) {
        super(context);
        init(context);
    }

    public Card_GasLock(Context context, DeviceInfo devicedata) {
        super(context);
    ...
    private void init(Context context){
        ...
        rootView = inflater.inflate(R.layout.device_gaslock, this); // from : 가져올 layout 지정
        ...
        m_btn_normalpower.setOnTouchListener(new OnTouchListener() {
            public boolean onTouch(View v, MotionEvent event) {
                if (event.getAction() == MotionEvent.ACTION_DOWN) v.setAlpha(0.5F);
                if (event.getAction() == MotionEvent.ACTION_UP || event.getAction() == MotionEvent.ACTION_CANCEL) {
                    if (m_received_Value) { //Lock
                        m_btn_normalpower.setTextColor(0x80ffffff);
                        m_btn_normalpower.setAlpha(0.5F);
                    } else { //Unlock
                        m_btn_normalpower.setTextColor(0xffffffff);
    ...
    private void updateDevice() {
        ...
        if (m_received_Value) { //Lock
            m_btn_normalpower.setTextColor(0x80ffffff);
            m_btn_normalpower.setAlpha(0.5F);
            m_text_value.setText(getResources().getText(R.string.card_gaslock_status_lock));
        } else { //Unlock
            m_btn_normalpower.setTextColor(0xffffffff);
            m_btn_normalpower.setAlpha(1.0F);
            m_text_value.setText(getResources().getText(R.string.card_gaslock_status_unlock));
        }
    ...
    public void BtnNormalpower_Clicked(View v)
    {
        ...
        Log.d(TAG, "v is selected " + v.isSelected());

        if(m_devicedata.getValue().equals("lock"))
        {
            MainActivity.getInstance().showToastBox(MainActivity.getInstance().getString(R.string.card_gaslock_not_use_open), true);
        }
        else
        {
            m_internal_Value = true;
            controlDevice(TypeDef.SWITCH_LOCK); //Lock만 가능함
            m_UserEventCnt++;
        }
    }
