 ...
 */
public class Card_SmartPlug_new extends LinearLayout implements View.OnClickListener {

    static final String TAG = "Card_SmartPlug";
    ...
    private float m_received_electricValue = 0;
    ...
    public Card_SmartPlug_new(Context context) {
        super(context);
        init(context);
    }

    public Card_SmartPlug_new(Context context, DeviceInfo devicedata) {
        super(context);
    ...
    private void init(Context context) {
        ...
        rootView = inflater.inflate(R.layout.device_smartplug, this); // from : 가져올 layout 지정
        ...
        m_text_unit = (TextView) rootView.findViewById(R.id.text_unit);
    ...
    private boolean getDeviceValue() {
        ...
        try {
            if (m_devicedata.getOtherValue(m_ID_switchbinary).equalsIgnoreCase(TypeDef.SWITCH_ON)) getvalue = true;
        } catch (Exception e) {
            //e.printStackTrace();
        }
        return getvalue;
    ...
    private float getDeviceEctricValue(int i) {
        int getvalue = 0;
        float getWvalue = 0;
        int getPrecision = 0;
        ...
        //todo 2016-12-08
        try {
            getWvalue = Float.parseFloat(m_devicedata.getOtherValue(i)); //0:electricMeter
            getPrecision = Integer.parseInt(m_devicedata.getOtherPrecision(i));
            if(getPrecision > 0) getWvalue = getWvalue/(float) Math.pow(10,getPrecision);

            Log.d(TAG, "getPrecision ... " + getPrecision + " , value : " + getWvalue);
        } catch (Exception e) {
            //e.printStackTrace();
        }
    ...
    private void updateDevice() {
        ...
        m_received_electricValue = getDeviceEctricValue(m_ID_electricMeter);
        m_text_title.setText(m_devicedata.getNickName());
        ...
        Log.d(TAG, "m_received_electricValue : " + m_received_electricValue);
        //String value = String.format("%.0f", m_received_electricValue);
        m_text_value.setText(String.valueOf(m_received_electricValue));
        //m_text_unit.setText("W"); //todo

        if (m_received_Value) { //ON
            m_btn_normalpower.setSelected(true);
//            m_text_value.setTextColor(getResources().getColor(R.color.data_number_text));
        }
        else
        { //Off
            m_btn_normalpower.setSelected(false);
//            m_text_value.setTextColor(getResources().getColor(R.color.data_number_text_disable));
        }
    ...
    */
    ...
    private int findDeviceControllerID(String subSort) {
        int index = -1;
        for(int i=0; i< m_devicedata.getOtherSubcount(); i++) {
            if (m_devicedata.getOtherSort(i).equalsIgnoreCase(subSort)) {
                index = i;
                break;
            }
        }
