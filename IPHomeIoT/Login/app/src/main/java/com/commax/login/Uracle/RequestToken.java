...
public class RequestToken {
    ...
    public void GetToken(HttpURLConnection urlConnection, String params[], Handler mHandler) {
        Log.d(TAG, "Get Token ");
        //parmas[0] :type , [1] :param1 , [2] : mac , [3] : model name , [4] : client_id , [5] :client_secret , [6] : MainActivity , SubActivity
        status = 0;
        try {
           ...
           /* String mac = aboutFile.readFile("mac_address");
                    ...
                    if (ret.length() > 0) {
                        ...
                        try {
                            ...
                            try
                            {
                                ...
                                turn = jsonObject1.getString(turn);
                                stun = jsonObject1.getString(stun);

                                Log.e(TAG, "ucServer :  ns/ews  =>" + ns +"/ "+ ews);
                                aboutFile.writeFile("ews",ews);
                                aboutFile.writeFile("ns",ns);
                                aboutFile.writeFile("turn",turn);
                                aboutFile.writeFile("stun",stun);
            ...
            e.printStackTrace();
            if(params[6].equals(TypeDef.SubActivity))
            {
                Message msg = mHandler.obtainMessage();
                msg.what = 2;
