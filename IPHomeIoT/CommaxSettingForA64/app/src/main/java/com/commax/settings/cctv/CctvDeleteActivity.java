 ...
 * CCTV 삭제 액티비티
 */
public class CctvDeleteActivity extends CommonActivity implements DeleteCctvListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_cctv_delete);
        setFullScreen();
        initListView();
    ...
    private void iselectAllbuttonToggle() {
        ...
        int count = ((DeleteCctvListAdapter) deviceList.getAdapter()).getCount();
        ...
        ((DeleteCctvListAdapter) deviceList.getAdapter()).notifyDataSetChanged();
        ...
        if (getCheckedDevices().size() > 0) {
            ...
            setScreenTitle(getString(R.string.title_delete_cctv));
        }
    }
    ...
    private void initListView() {
        ...
        if (TypeDef.OP_NEWDELETE_ADAPTOR_ENABLE) { //2017-01-23,yslee:: 신규 Delete UX 적용
            DeleteCctvListAdapter adapter = new DeleteCctvListAdapter(this, R.layout.list_item_delete_cctv2, devices);
            deviceList.setChoiceMode(ListView.CHOICE_MODE_MULTIPLE);
            deviceList.setAdapter(adapter);

            hideSaveDeleteButton();
        } else {
            DeleteCctvListAdapter adapter = new DeleteCctvListAdapter(this, R.layout.list_item_delete_cctv, devices);
            deviceList.setAdapter(adapter);
        }
    ...
    private ArrayList<OnvifDevice> getCheckedDevices() {
        final ListView deviceList = (ListView) findViewById(R.id.registeredCctvList);
        ArrayList<OnvifDevice> devices = ((DeleteCctvListAdapter) deviceList.getAdapter()).getDevices();
    ...
    private ArrayList<OnvifDevice> doRemoveCheckedDevices() {
        ...
        if(size > 0) {
            for (int i = size-1; i >= 0; i--) {
                ...
                ((DeleteCctvListAdapter) deviceList.getAdapter()).delDevice(key);
            }
        }
    ...
    private void showDeletePopup(OnvifDevice onvifDevice) {
        CctvDeletePopup popup = new CctvDeletePopup(this, this);
        if (!TypeDef.OP_NEWDELETE_ADAPTOR_ENABLE) {
    ...
    private void performSaveDelete() {
        ...
        if (getCheckedDevices().size() == 0) {
            Toast.makeText(CctvDeleteActivity.this, R.string.choose_camera, Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        for (OnvifDevice device : devices) {
            ...
            //도어카메라 삭제
            ContentProviderManager.deleteOnvifCctv(this, device.getIpAddress());
        }
        ...
        Toast.makeText(CctvDeleteActivity.this, R.string.deleted, Toast.LENGTH_SHORT).show();
    }
    /**
     * 카메라 데이터 테스트용
     *
    ...
    private void refreshList(OnvifDevice device) {
        final ListView deviceList = (ListView) findViewById(R.id.registeredCctvList);
        ((DeleteCctvListAdapter) deviceList.getAdapter()).deleteDevice(device);
